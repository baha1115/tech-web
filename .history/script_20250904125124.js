...
let glitchtl = gsap.timeline({ repeat: -1 })
...
confused.addEventListener('click',e=>{
   ...
   clicktl.from(begining,{
    opacity:0,height:0,delay:2,duration:2
   })
   clicktl.to({onStart:()=>{
    begining.style.display='none'
   }})
   clicktl.to(confusedPage,{onStart:()=>{
    confusedPage.style.display='block'
    glitchback.forEach(back => {
  glitchtl.to('.confused-page', {
    backgroundImage: `url(${back})`,
    duration: 0.1, // سريع يعطي إحساس glitch
    repeat: 2,     // يكرر نفس الصورة شوي
    yoyo: true
  }, "+=0.05") // فاصلة صغيرة بين التغييرات
})
   },duration:4})
