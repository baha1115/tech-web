  ...
  //section2
    ...
    gsap.from('.si',{
      opacity:0,y:10,stagger:0.2,scrollTrigger:{
        trigger:'.section2-part2',start:'top 110%'
      }
    })
   // الكلمات أو الرموز اللي تعمل بيها "قليتش"
let hackeffecttitle2 = ['#@$%', 'X0_1', 'LOST', '###', 'A9@', 'H@CK'];

document.querySelectorAll('.services-title').forEach(serv => {
  let originalText = serv.textContent; // النص الأصلي

  serv.addEventListener('mouseenter', e => {
    // نوقف أي أنيميشن قديم
    if (serv.timeline) serv.timeline.kill();
