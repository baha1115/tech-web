...
confused.addEventListener('click', e => {
  ...
  confusedPage.style.display = 'none'
  ...
  // يخفي البداية ويظهر صفحة الغليتش
  clicktl.to(begining, {
    opacity: 0,
    duration: 0.3,
    onComplete: () => {
      begining.style.display = 'none'
      confusedPage.style.display = 'block'
    }
  })

  // نطلق glitch animation
  clicktl.add(glitchAnim(confusedPage), ">")
})

function glitchAnim(target) {
  let glitchtl = gsap.timeline({ repeat: 2 }) // يكرر 2 مرات
  ...
  glitchback.forEach((back, i) => {
    glitchtl.to(target, {
      duration: 0.08,
      backgroundImage: `url(${back})`,
      x: () => gsap.utils.random(-15, 15),   // اهتزاز أفقي
      y: () => gsap.utils.random(-10, 10),   // اهتزاز عمودي
      skewX: () => gsap.utils.random(-10, 10), // انكسار
      scale: () => gsap.utils.random(0.95, 1.05), // تكبير/تصغير خفيف
      ease: "steps(1)" // حركات متقطعة (glitch)
    })
  })
  ...
  glitchtl.to(target, {
    duration: 0.2,
    backgroundImage: `url(${glitchback[0]})`,
    x: 0,
    y: 0,
    skewX: 0,
    scale: 1,
